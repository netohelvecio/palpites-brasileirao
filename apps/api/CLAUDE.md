...
database/
  migrations/      # 0001_..NNNN_ — numeradas manualmente para ordem explícita
...
- **Identidade do user no WhatsApp**: `users` tem duas colunas nullable — `whatsapp_number` (telefone E.164) e `whatsapp_lid` (JID `@lid`; DMs com privacidade do remetente entram assim). Se `m.key.senderPn` resolve pro telefone depende da sessão Baileys atual, não da versão da lib — um re-pareamento pode flipar esse comportamento em qualquer direção, então não assuma que um dos dois formatos é o "normal". `UserRepository.findByWhatsappIdentity(phone, jid)` busca por `OR` nas duas colunas. `WhatsAppInboundHandler.resolveUser`/`healIdentity` fazem auto-cura: preenchem a coluna que estiver `null` com o valor recém-visto na mensagem — convergindo só pras identidades que a sessão atual efetivamente entrega — e, se a coluna já tinha valor divergente do que chegou, **só logam warn** — nunca sobrescrevem. O write da auto-cura roda dentro do seu próprio `db.transaction` **de propósito**, mesmo sendo um `UPDATE` de statement único: é best-effort, e uma violação de unique (duas linhas vivas da mesma pessoa) aborta a transação no Postgres — `try/catch` sozinho não despolui, só `ROLLBACK TO SAVEPOINT`. Sem o savepoint, a falha derrubaria a mensagem inteira do usuário. Hoje nenhum caller envolve o handler numa transação, então em produção é inerte; o que ele garante é que continue assim se alguém unificar upsert + heal numa transação só. Se a sessão parar de resolver `senderPn`, um user cadastrado só por lid não aprende o telefone sozinho: backfill é manual (`node ace whatsapp:lookup-lid <phone-E.164>` + `UPDATE users SET whatsapp_number = ...`; o comando não roda em produção com a API no ar — abre uma segunda sessão Baileys contra o mesmo auth multi-file e quebra o pareamento). Índices únicos são parciais (`CREATE UNIQUE INDEX ... WHERE is_deleted = false`, migration `0013_add_whatsapp_lid_to_users.ts`) pra permitir reaproveitar telefone/lid depois de soft-delete. Outbound usa o getter `User.whatsappTarget` (`whatsapp_lid ?? whatsapp_number`, getter plano — não `@computed`, pra não vazar em respostas HTTP). **A precedência é o lid de propósito**: é o endereço provado em produção pros contatos de privacidade-lid, enquanto o JID de telefone nunca foi exercitado com eles e uma entrega que falha some sem erro (o Baileys não lança). Preferimos o caminho conhecido — `OpenRoundJob` pula (com warn) users sem nenhuma identidade. `sendToUser` detecta `@` na string e usa direto como JID, então `whatsappTarget` funciona tanto pra telefone quanto pra lid. Pra descobrir o lid de um phone conhecido: `node ace whatsapp:lookup-lid <phone-E.164>` chama `socket.onWhatsApp()` e devolve `{ jid, lid, exists }`.
