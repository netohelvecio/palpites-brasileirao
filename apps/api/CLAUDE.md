...
- **Identidade do user no WhatsApp**: `users` tem duas colunas nullable — `whatsapp_number` (telefone E.164) e `whatsapp_lid` (JID `@lid`; DMs com privacidade do remetente entram assim, geralmente sem `m.key.senderPn` resolvível em Baileys 6.7.x). Um user pode existir só com uma das duas até a primeira DM resolver a outra. `UserRepository.findByWhatsappIdentity(phone, jid)` busca por `OR` nas duas colunas. `WhatsAppInboundHandler.resolveUser`/`healIdentity` fazem auto-cura: preenchem a coluna que estiver `null` com o valor recém-visto e, se a coluna já tinha valor divergente do que chegou, **só logam warn** — nunca sobrescrevem. Índices únicos são parciais (`CREATE UNIQUE INDEX ... WHERE is_deleted = false`, migration `0013_add_whatsapp_lid_to_users.ts`) pra permitir reaproveitar telefone/lid depois de soft-delete. Outbound usa o getter `User.whatsappTarget` (`whatsapp_number ?? whatsapp_lid`, getter plano — não `@computed`, pra não vazar em respostas HTTP) — `OpenRoundJob` pula (com warn) users sem nenhuma identidade. `sendToUser` detecta `@` na string e usa direto como JID, então `whatsappTarget` funciona tanto pra telefone quanto pra lid. Pra descobrir o lid de um phone conhecido: `node ace whatsapp:lookup-lid <phone-E.164>` chama `socket.onWhatsApp()` e devolve `{ jid, lid, exists }`.
