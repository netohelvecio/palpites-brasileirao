...
| Plano 9 (9.1–9.5) | ✅ concluído | Tied Featured Match Poll — quando o picker empata no top sem que seja 1×2, sync persiste candidatos em `round_match_candidates` e flipa rodada pra `awaiting_pick`; `OpenRoundJob` manda enquete WhatsApp (poll nativo + fallback emoji); admin homologa via `POST /rounds/:id/pick-candidate` ou `/escolher <n>` no DM. `RoundCandidatePickService` é transacional (forUpdate, soft-delete dos candidatos). Spec/planos em `docs/superpowers/specs/2026-05-07-tied-featured-match-poll-design.md` e `docs/superpowers/plans/2026-05-07-plan-9.*.md` |
